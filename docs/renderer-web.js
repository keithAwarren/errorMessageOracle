// ======= Config =======
const API_URL = "https://erroracle-backend.onrender.com/oracle";
...
let emptySummonCount = 0; // Count summons without offerings
let currentOracleAbort = null; // AbortController for the current rite
...
// Call the Oracle over HTTP
async function askOracle(inputValue, signal) {
  const res = await fetch(API_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ errorText: inputValue }),
    signal,
  });

  if (!res.ok) {
    // Try to extract server error; otherwise generic
    let msg = "The Oracle is having visions. Try again later.";
    try {
...
async function invokeOracle() {
  ...
  // Empty offering: admonishment—or wrath, if they persist
  if (!inputValue) {
    emptySummonCount++;
  ...
  // Reset the count on a worthy sacrifice
  emptySummonCount = 0;
  ...
  try {
    const response = await askOracle(inputValue, currentOracleAbort.signal);
    ...
    if (err && err.name === "AbortError") return; // canceled is fine
...
document.getElementById("clearButton").addEventListener("click", () => {
  ...
  emptySummonCount = 0;
  lastOracle = { poetic: "", plain: "", hasContent: false };
...
boringButton.addEventListener("click", () => {
  ...
  document.getElementById("summonButton").textContent = isBoring ? "Submit" : "Summon Oracle";
  document.getElementById("clearButton").textContent = isBoring ? "Clear" : "Cleanse";
